...
function drawSky(pc) {
  ...
  if (pc.night > 0.01) {
    ...
    // Corona glow (radial gradient)
    const glow = cx.createRadialGradient(sx, sy, 8, sx, sy, 45);
    glow.addColorStop(0, 'rgba(255, 235, 170, 0.45)');
    glow.addColorStop(1, 'rgba(255, 235, 170, 0)');
    cx.fillStyle = glow;
    cx.beginPath(); cx.arc(sx, sy, 45, 0, 2 * Math.PI); cx.fill();
    // Solid sun core
    cx.fillStyle = `rgba(255, 239, 175, ${sunOpacity})`;
    cx.beginPath(); cx.arc(sx, sy, 11, 0, 2 * Math.PI); cx.fill();
  }
...
function cloud(x, y, r, rgb, a) {
  ...
  cx.fillStyle = `rgba(${rgb}, ${a * 0.9})`;
  cx.beginPath();
  cx.arc(x - r * 0.35, y + r * 0.1, r * 0.45, 0, 2 * Math.PI);
  cx.arc(x, y - r * 0.1, r * 0.6, 0, 2 * Math.PI);
  cx.arc(x + r * 0.4, y, r * 0.5, 0, 2 * Math.PI);
  cx.closePath(); cx.fill();
}
  ...
  for (let ty = y0; ty <= y1; ty++) {
    for (let tx = x0; tx <= x1; tx++) {
      ...
      if (t === 1) {
        ...
        const rockColor = hexLerp(ROCKC[bio], jit > 0 ? '#cbc7b9' : '#6e6a5f', Math.abs(jit) * 0.12);
        ...
        if (leftAir) { cx.fillStyle = 'rgba(0,0,0,0.06)'; cx.fillRect(x, y, 2, TILE); }
        ...
        const rockColor = hexLerp('#b3a88e', dh > 0.5 ? '#c6bca2' : '#928871', Math.abs(dh - 0.5) * 0.25);
        cx.fillStyle = rockColor;
        cx.fillRect(x - SEAM, y - SEAM, TILE + SEAM * 2, TILE + SEAM * 2);
...
function drawFlower(fx, fr, kind) {
  ...
  cx.beginPath(); cx.moveTo(x, y); cx.lineTo(x, y - 10); cx.stroke();

  // Small green leaves on stem
  cx.fillStyle = '#557d3a';
  cx.beginPath();
  cx.ellipse(x - 2, y - 5, 2, 1, -0.4, 0, 2 * Math.PI);
  cx.ellipse(x + 2, y - 6, 2, 1, 0.4, 0, 2 * Math.PI);
  cx.fill();

  // Draw flower head
  const fx_c = x, fy_c = y - 11;

  if (kind === 'gent') {
    // Gentian ( trumpet-shaped blue bell flower )
    const bellColor = hexLerp('#2b5fb8', '#0b1f58', curPC.night);
    const innerColor = hexLerp('#6b9bf8', '#254ba0', curPC.night);
    
    cx.fillStyle = bellColor;
    cx.beginPath();
    cx.moveTo(fx_c - 4, fy_c + 2);
    cx.quadraticCurveTo(fx_c - 5, fy_c - 6, fx_c - 7, fy_c - 10);
    cx.lineTo(fx_c - 3, fy_c - 8);
    cx.lineTo(fx_c, fy_c - 12);
    cx.lineTo(fx_c + 3, fy_c - 8);
    cx.lineTo(fx_c + 7, fy_c - 10);
    cx.quadraticCurveTo(fx_c + 5, fy_c - 6, fx_c + 4, fy_c + 2);
    cx.closePath(); cx.fill();
    
    // Inner lighter blue throat
    cx.fillStyle = innerColor;
    cx.beginPath();
    cx.moveTo(fx_c - 3, fy_c - 4);
    cx.lineTo(fx_c, fy_c - 8);
    cx.lineTo(fx_c + 3, fy_c - 4);
    cx.lineTo(fx_c, fy_c - 1);
    cx.closePath(); cx.fill();
  } else if (kind === 'rose') {
    // Alpine Wild Rose (pink petals, dark pink center, yellow core)
    const petalColor = hexLerp('#d9577a', '#4a1b26', curPC.night);
    const darkCenter = hexLerp('#b53c5a', '#360f18', curPC.night);
    const yellowCore = hexLerp('#ffd54f', '#786220', curPC.night);
    
    cx.fillStyle = petalColor;
    for (let i = 0; i < 5; i++) {
      const a = (i / 5) * Math.PI * 2;
      cx.beginPath();
      cx.arc(fx_c + Math.cos(a) * 2.5, fy_c + Math.sin(a) * 2.5, 2.2, 0, 2 * Math.PI);
      cx.fill();
    }
    
    cx.fillStyle = darkCenter;
    cx.beginPath();
    cx.arc(fx_c, fy_c, 2, 0, 2 * Math.PI);
    cx.fill();
    
    cx.fillStyle = yellowCore;
    cx.beginPath();
    cx.arc(fx_c, fy_c, 1, 0, 2 * Math.PI);
    cx.fill();
  } else {
    // Edelweiss (star-shaped pointed white petals, yellow center)
    const whitePetal = hexLerp('#ffffff', '#5c5c5c', curPC.night);
    const centerColor = hexLerp('#ffd54f', '#786220', curPC.night);
    
    cx.fillStyle = whitePetal;
    for (let i = 0; i < 8; i++) {
      const a = (i / 8) * Math.PI * 2;
      cx.save();
      cx.translate(fx_c, fy_c);
      cx.rotate(a);
      cx.beginPath();
      cx.moveTo(0, -5);
      cx.lineTo(2, 0);
    ...
    cx.beginPath();
    cx.arc(fx_c, fy_c, 1.2, 0, 2 * Math.PI);
    cx.fill();
  }
}
...
function drawEntity(e) {
  ...
  switch (e.t) {
    ...
    case 'cow': {
      ...
      // 1. Draw Far Legs (drawn behind body, shaded darker cream)
      cx.fillStyle = '#d4cebe';
      cx.fillRect(-6.5 - swing, -5, 3, 5); // far rear leg
      cx.fillRect(3 + swing, -5, 3, 5);    // far front leg
      // Far Hooves
      cx.fillStyle = '#1e1e1a';
      cx.fillRect(-7 - swing, -1.5, 4, 1.5);
      cx.fillRect(2.5 + swing, -1.5, 4, 1.5);

      // 2. Draw Udder (pinkish-tan)
      cx.fillStyle = '#cca489';
      cx.beginPath();
      cx.arc(-1.5, -4.5, 2.2, 0, Math.PI);
      cx.fill();
      // Udder teats
      cx.fillRect(-2.5, -2.5, 0.8, 1.2);
      cx.fillRect(-0.8, -2.5, 0.8, 1.2);

      // 3. Draw Body (cream base, rounded corners)
      cx.fillStyle = '#eae4d3';
      cx.beginPath();
      cx.moveTo(-10, -14);
      cx.lineTo(10, -14);
      cx.quadraticCurveTo(12, -14, 12, -12);
      cx.lineTo(12, -6.5);
      cx.quadraticCurveTo(12, -4.5, 10, -4.5);
      cx.lineTo(-10, -4.5);
      cx.quadraticCurveTo(-12, -4.5, -12, -6.5);
      cx.lineTo(-12, -12);
      cx.quadraticCurveTo(-12, -14, -10, -14);
      cx.closePath();
      cx.fill();

      // 4. Draw Body Patches (brown)
      cx.fillStyle = '#9a6f4a';
      // Flank/rear patch
      cx.beginPath();
      cx.moveTo(-10, -14);
      cx.quadraticCurveTo(-4, -14, -4, -9);
      cx.quadraticCurveTo(-4, -4.5, -9, -4.5);
      cx.lineTo(-10, -4.5);
      cx.quadraticCurveTo(-12, -4.5, -12, -6.5);
      cx.lineTo(-12, -12);
      cx.quadraticCurveTo(-12, -14, -10, -14);
      ...
      // Shoulder/front patch
      cx.beginPath();
      cx.moveTo(2, -14);
      cx.quadraticCurveTo(2, -9, 4, -4.5);
      cx.lineTo(10, -4.5);
      cx.quadraticCurveTo(12, -4.5, 12, -6.5);
      cx.lineTo(12, -12);
      cx.quadraticCurveTo(12, -14, 10, -14);
      ...
      // 5. Draw Tail (brown)
      cx.strokeStyle = '#9a6f4a'; cx.lineWidth = 1.2;
      cx.beginPath();
      cx.moveTo(-11, -12);
      cx.quadraticCurveTo(-13, -8, -12, -4 + (e.vx ? Math.sin(frame * 0.1) * 1 : 0));
      cx.stroke();
      cx.fillStyle = '#9a6f4a';
      cx.beginPath();
      cx.ellipse(-12, -3 + (e.vx ? Math.sin(frame * 0.1) * 1 : 0), 1.2, 2.2, 0, 0, 7);
      cx.fill();
      ...
      // 7. Draw Rear Horn (behind head)
      cx.fillStyle = '#eae4d3';
      cx.beginPath();
      cx.moveTo(11, -17.5);
      cx.quadraticCurveTo(10, -22, 11.5, -22.5);
      cx.quadraticCurveTo(12.5, -22, 12.5, -17.5);
      cx.closePath(); cx.fill();

      // 8. Draw Rear Ear (behind head)
      cx.fillStyle = '#eae4d3';
      cx.beginPath(); cx.ellipse(10.5, -14.5, 2.5, 1.3, -0.4, 0, 7); cx.fill();
      cx.fillStyle = '#9a6f4a';
      cx.beginPath(); cx.ellipse(10.8, -14.3, 1.8, 0.8, -0.4, 0, 7); cx.fill();

      // 9. Draw Head (brown)
      cx.fillStyle = '#9a6f4a';
      cx.beginPath(); cx.arc(14, -14, 4.5, 0, 7); cx.fill();

      // 10. Draw Front Ear (in front of head)
      cx.fillStyle = '#eae4d3';
      cx.beginPath(); cx.ellipse(17.5, -14.5, 2.5, 1.3, 0.4, 0, 7); cx.fill();
      cx.fillStyle = '#9a6f4a';
      cx.beginPath(); cx.ellipse(17.2, -14.3, 1.8, 0.8, 0.4, 0, 7); cx.fill();

      // 11. Draw Front Horn (in front of head)
      cx.fillStyle = '#eae4d3';
      cx.beginPath();
      cx.moveTo(15.5, -17.5);
      cx.quadraticCurveTo(15, -22, 16.5, -22.5);
      cx.quadraticCurveTo(17.5, -22, 17, -17.5);
      cx.closePath(); cx.fill();

      // 12. Draw Muzzle/Snout (cream)
      cx.fillStyle = '#eae4d3';
      cx.beginPath(); cx.ellipse(14.5, -12.5, 3.5, 2.3, 0, 0, 7); cx.fill();

      // 13. Draw Nostrils (dark)
      cx.fillStyle = '#2c251f';
      cx.beginPath();
      cx.arc(13.2, -12.5, 0.5, 0, 7);
      cx.arc(15.8, -12.5, 0.5, 0, 7);
      cx.fill();

      // 14. Draw Eyes (dark ovals)
      cx.beginPath();
      cx.ellipse(12.2, -16, 0.7, 1.1, 0, 0, 7);
      cx.ellipse(15.2, -16, 0.7, 1.1, 0, 0, 7);
      cx.fill();

      // 15. Draw Collar & Bell
      cx.fillStyle = '#453c32'; // collar
      cx.beginPath();
      cx.ellipse(10.8, -11.5, 1.2, 3.2, 0.3, 0, 7);
      cx.fill();
      cx.fillStyle = '#a5bcc4'; // steel-blue bell
      cx.beginPath();
      cx.moveTo(8.5, -8.5);
      cx.lineTo(10.5, -8.5);
      cx.lineTo(11.5, -5.5);
      cx.lineTo(7.5, -5.5);
      cx.closePath(); cx.fill();
      cx.fillStyle = '#e9b646'; // gold rim
      cx.fillRect(7.2, -5.5, 4.6, 0.8);

      // 16. Draw Near Legs (drawn in front of body)
      cx.fillStyle = '#eae4d3';
      cx.fillRect(-11 + swing, -5, 3, 5); // near rear leg
      cx.fillRect(7.5 - swing, -5, 3, 5);  // near front leg
      // Near Hooves
      cx.fillStyle = '#1e1e1a';
      cx.fillRect(-11.5 + swing, -1.5, 4, 1.5);
      cx.fillRect(7 - swing, -1.5, 4, 1.5);
...
function drawIcon(name, x, y, s) {
  ...
  switch (name) {
    ...
    case 'book': {
      // Warm brown cover with curved arches and dip at the spine
      cx.fillStyle = '#7a5a39';
      cx.beginPath();
      cx.moveTo(0, -3.5);
      cx.quadraticCurveTo(-4.5, -7.5, -9, -6.5);
      cx.lineTo(-9, 5.5);
      cx.quadraticCurveTo(-4.5, 4.5, 0, 8.0);
      cx.quadraticCurveTo(4.5, 4.5, 9, 5.5);
      cx.lineTo(9, -6.5);
      cx.quadraticCurveTo(4.5, -7.5, 0, -3.5);
      cx.closePath();
      cx.fill();

      // Cream pages left and right with margins
      cx.fillStyle = '#fbf8f0';
      cx.beginPath();
      cx.moveTo(0, -4.5);
      cx.quadraticCurveTo(-4, -7.8, -7.5, -6.8);
      cx.lineTo(-7.5, 4.2);
      cx.quadraticCurveTo(-4, 3.2, 0, 6.5);
      cx.closePath();
      cx.fill();
      ...
      // Flat tan vertical spine bar
      cx.strokeStyle = '#bcae88';
      cx.lineWidth = 1.8;
      cx.lineCap = 'round';
      cx.beginPath();
      cx.moveTo(0, -4);
      ...
      // Flat tan horizontal text pill-shapes
      cx.lineWidth = 1.5;
      cx.beginPath();
      cx.moveTo(-5.8, -2.5); cx.lineTo(-2.2, -2.5);
      cx.moveTo(-5.8, 1.5); cx.lineTo(-2.2, 1.5);
      cx.moveTo(2.2, -2.5); cx.lineTo(5.8, -2.5);
      cx.moveTo(2.2, 1.5); cx.lineTo(5.8, 1.5);
      cx.stroke();
      break;
    }
    case 'camera': {
    ...
    case 'map_flat': {
      cx.strokeStyle = '#3a5e3a'; cx.lineWidth = 1.8;
      cx.beginPath();
      cx.moveTo(-6, -5); cx.lineTo(-2, -7); cx.lineTo(2, -5); cx.lineTo(6, -7);
      cx.lineTo(6, 5); cx.lineTo(2, 7); cx.lineTo(-2, 5); cx.lineTo(-6, 7);
      cx.closePath();
      cx.stroke();
      cx.fillStyle = 'rgba(58, 94, 58, 0.15)';
      cx.fill();
      cx.beginPath();
      cx.moveTo(-2, -7); cx.lineTo(-2, 5);
      cx.moveTo(2, -5); cx.lineTo(2, 7);
      cx.stroke();
      break;
    }
    case 'backpack_flat': {
      ...
      cx.beginPath();
      cx.moveTo(-5, -5);
      cx.quadraticCurveTo(-5, -7, -3, -7);
      cx.lineTo(3, -7);
      cx.quadraticCurveTo(5, -7, 5, -5);
      cx.lineTo(5, 6);
      cx.quadraticCurveTo(5, 8, 3, 8);
      cx.lineTo(-3, 8);
      cx.quadraticCurveTo(-5, 8, -5, 6);
      cx.closePath(); cx.fill();
      cx.fillStyle = '#f2ebd5';
      cx.fillRect(-3, -1, 6, 6);
      cx.fillStyle = '#3a5e3a';
      cx.fillRect(-1.5, 1.5, 3, 2.5);
      break;
    }
    case 'book_flat': {
      cx.fillStyle = '#3a5e3a';
      cx.beginPath();
      cx.moveTo(0, 4);
      cx.quadraticCurveTo(-4, 0, -8, 2);
      cx.lineTo(-8, -6);
      cx.quadraticCurveTo(-4, -8, 0, -4);
      cx.closePath(); cx.fill();
      cx.beginPath();
      cx.moveTo(0, 4);
      cx.quadraticCurveTo(4, 0, 8, 2);
    ...
    case 'gear_flat': {
      cx.fillStyle = '#3a5e3a';
      cx.beginPath();
      cx.arc(0, 0, 4.5, 0, 2 * Math.PI);
      cx.fill();
      for (let i = 0; i < 8; i++) {
        const a = (i / 8) * Math.PI * 2;
        cx.save(); cx.rotate(a);
        cx.fillRect(-1.5, -7, 3, 3);
        cx.restore();
      }
      cx.fillStyle = '#f2ebd5';
      cx.beginPath(); cx.arc(0, 0, 1.8, 0, 2 * Math.PI); cx.fill();
      break;
    }
    case 'x_flat': {
      cx.strokeStyle = '#3a5e3a'; cx.lineWidth = 2.2;
      cx.beginPath(); cx.moveTo(-4.5, -4.5); cx.lineTo(4.5, 4.5); cx.moveTo(4.5, -4.5); cx.lineTo(-4.5, 4.5); cx.stroke();
      break;
    }
    case 'expand_flat': case 'shrink_flat': {
      cx.strokeStyle = '#3a5e3a'; cx.lineWidth = 1.8;
      const out = name === 'expand_flat';
      for (const [sx2, sy2] of [[-1, -1], [1, -1], [-1, 1], [1, 1]]) {
        const tipX = sx2 * (out ? 6 : 1.8), tipY = sy2 * (out ? 6 : 1.8);
        const tailX = sx2 * (out ? 1.8 : 6), tailY = sy2 * (out ? 1.8 : 6);
        const hd = out ? sx2 * 3 : -sx2 * 3, hv = out ? sy2 * 3 : -sy2 * 3;
        cx.beginPath();
        cx.moveTo(tailX, tailY); cx.lineTo(tipX, tipY);
        cx.moveTo(tipX - hd, tipY); cx.lineTo(tipX, tipY); cx.lineTo(tipX, tipY - hv);
...
function drawPortrait(who, x, y, s) {
  ...
  switch (who) {
    ...
    case 'cow': {
      ...
      // Cow leather collar & steel-blue bell
      cx.fillStyle = '#453c32'; // dark collar
      cx.beginPath();
      ...
      // Gold clapper/bottom rim
      cx.fillStyle = '#e9b646';
      cx.fillRect(-3.5, 18, 7, 1.2);

      // Cow Head (pointy ears on sides)
      cx.fillStyle = '#9a6f4a';
      cx.beginPath(); cx.arc(0, hy - 1, 8.2, 0, 7); cx.fill();

      // Ears (sideways, drooping, cream outer, brown inner)
      cx.fillStyle = '#eae4d3';
      ...
      // Nostril slits (dark brown/charcoal)
      cx.fillStyle = '#2c251f';
      cx.beginPath(); cx.ellipse(-2.2, hy + 4.4, 0.8, 1.4, 0.2, 0, 7); cx.fill();
      cx.beginPath(); cx.ellipse(2.2, hy + 4.4, 0.8, 1.4, -0.2, 0, 7); cx.fill();

      // Large gentle eyes (vertical ovals)
      cx.fillStyle = '#2c251f';
      cx.beginPath();
      cx.ellipse(-3.2, hy - 2.8, 1.2, 2.0, 0, 0, 7);
      cx.ellipse(3.2, hy - 2.8, 1.2, 2.0, 0, 0, 7);
      cx.fill();
      break;
    }
...
function drawBtn(b) {
  ...
  const isMenuBtn = (b.id === 'map' || b.id === 'mute' || b.id === 'fs' || b.id === 'albumBack');
  ...
  if (isMenuBtn) {
    ...
    // Touch controls (jump / act) - semi-transparent white style
    cx.fillStyle = on ? 'rgba(255, 255, 255, 0.45)' : 'rgba(255, 255, 255, 0.22)';
    cx.beginPath(); cx.arc(b.lx, b.ly, b.lr, 0, 2 * Math.PI); cx.fill();
    cx.strokeStyle = 'rgba(255, 255, 255, 0.5)'; cx.lineWidth = 2;
    cx.beginPath(); cx.arc(b.lx, b.ly, b.lr, 0, 2 * Math.PI); cx.stroke();
    
    if (b.label && b.label.startsWith('i:')) {
      drawIcon(b.label.slice(2), b.lx, b.ly, b.lr * 1.15);
    } else {
      cx.fillStyle = '#ffffff';
      cx.font = `bold ${Math.round(b.lr * 0.7)}px "Outfit", sans-serif`;
      cx.textAlign = 'center';
      cx.fillText(b.label, b.lx, b.ly + 1);
    }
  }
...
function drawJoystick(jR) {
  ...
  // base well: semi-transparent white outer circle
  cx.fillStyle = 'rgba(255, 255, 255, 0.18)';
  cx.beginPath(); cx.arc(bx, by, jR, 0, 2 * Math.PI); cx.fill();
  cx.strokeStyle = 'rgba(255, 255, 255, 0.45)'; cx.lineWidth = 2;
  cx.beginPath(); cx.arc(bx, by, jR, 0, 2 * Math.PI); cx.stroke();
  ...
  for (let i = 0; i < 4; i++) {
    ...
    cx.fillStyle = pushed ? 'rgba(255, 255, 255, 0.9)' : 'rgba(255, 255, 255, 0.35)';
    const tx = bx + dx * (jR - 10), ty = by + dy * (jR - 10);
    cx.save(); cx.translate(tx, ty); cx.rotate(a);
    cx.beginPath(); cx.moveTo(3, 0); cx.lineTo(-2, -3.5); cx.lineTo(-2, 3.5); cx.closePath(); cx.fill();
    cx.restore();
  }
  // knob: smaller inner circle
  cx.fillStyle = live ? 'rgba(255, 255, 255, 0.55)' : 'rgba(255, 255, 255, 0.3)';
  cx.beginPath(); cx.arc(kx, ky, knobR, 0, 2 * Math.PI); cx.fill();
  cx.strokeStyle = 'rgba(255, 255, 255, 0.7)'; cx.lineWidth = 2;
  cx.beginPath(); cx.arc(kx, ky, knobR, 0, 2 * Math.PI); cx.stroke();
}
...
function drawDialog(W, H) {
  ...
  if (pk) {
    ...
    // Clip the portrait inside a clean rounded-rectangle directly on the background
    cx.save();
    cx.beginPath();
    roundRect(px0, py0, ps, ps, 6);
    cx.clip();
    drawPortrait(pk, px0 + ps / 2, py0 + ps / 2, ps);
    cx.restore();
  ...
  if (cur[0]) {
    setTracking(1.8);
    cx.fillStyle = journal ? '#5a3f25' : '#3a5e3a';
    cx.font = journal ? 'bold 13.5px "Lora", Georgia, serif' : 'bold 13.5px "Outfit", sans-serif';
    cx.textAlign = 'left';
  ...
  cx.fillStyle = journal ? '#2c1e12' : '#223322';
  ...
  // once the line has finished typing: bobbing arrow
  if (D.chars >= cur[1].length) {
    // Bouncing chevron in bottom right corner
    const arrowY = by + bh - 18 + Math.sin(frame * 0.12) * 2;
    cx.strokeStyle = journal ? 'rgba(122, 90, 57, 0.8)' : '#3a5e3a';
    cx.lineWidth = 2;
    cx.lineCap = 'round';
    cx.lineJoin = 'round';
    cx.beginPath();
    cx.moveTo(bx + bw - 22 - 4, arrowY - 2);
    cx.lineTo(bx + bw - 22, arrowY + 2);
    cx.lineTo(bx + bw - 22 + 4, arrowY - 2);
    cx.stroke();
  }
}
...
const FLAT_PALETTES = [
  null,
  // Phase 1: Morning Mist
  {
    sky: '#d2e5f0', sun: '#fef4cf', moon: false,
    mountainBack: '#b4cdc3', mountainFront: '#88aaa0',
    hillBack: '#8fb56f', hillMid: '#7fa55f', hillFront: '#4f7c4c',
    conifer: '#416752', deciduous: '#6fa364', trunk: '#72543e'
  },
  // Phase 2: Rain
  {
    sky: '#a5b7c4', sun: false, moon: false,
    mountainBack: '#8ba198', mountainFront: '#6c887d',
    hillBack: '#729058', hillMid: '#62804c', hillFront: '#3d613a',
    conifer: '#325040', deciduous: '#547d4c', trunk: '#5c4331'
  },
  // Phase 3: Night
  {
    sky: '#0f172a', sun: false, moon: '#fef6d0',
    mountainBack: '#1d2a3a', mountainFront: '#111c2a',
    hillBack: '#142718', hillMid: '#0f1f12', hillFront: '#09150a',
    conifer: '#0b1b14', deciduous: '#112211', trunk: '#1c140e'
  },
  // Phase 4: First Light
  {
    sky: '#fed0b8', sun: '#ffa380', moon: false,
    mountainBack: '#b29da4', mountainFront: '#8c7784',
    hillBack: '#8f775c', hillMid: '#7e664e', hillFront: '#4c4230',
    conifer: '#4a3b3f', deciduous: '#604f58', trunk: '#4f392b'
  },
  // Phase 5: Clear Morning
  {
    sky: '#d2e5f0', sun: '#fef4cf', moon: false,
    mountainBack: '#b4cdc3', mountainFront: '#88aaa0',
    hillBack: '#8fb56f', hillMid: '#7fa55f', hillFront: '#4f7c4c',
...
function getFlatColors() {
  const a = FLAT_PALETTES[phasePrev] || FLAT_PALETTES[G.phase] || FLAT_PALETTES[1];
  const b = FLAT_PALETTES[G.phase] || FLAT_PALETTES[1];
  
  const lerpColor = (c1, c2) => {
    if (!c1) return c2;
    if (!c2) return c1;
    return hexLerp(c1, c2, phaseLerpT);
  };
  
  return {
    sky: lerpColor(a.sky, b.sky),
    sun: b.sun ? lerpColor(a.sun || b.sun, b.sun) : null,
    moon: b.moon ? lerpColor(a.moon || b.moon, b.moon) : null,
    mountainBack: lerpColor(a.mountainBack, b.mountainBack),
    mountainFront: lerpColor(a.mountainFront, b.mountainFront),
    hillBack: lerpColor(a.hillBack, b.hillBack),
    hillMid: lerpColor(a.hillMid, b.hillMid),
    hillFront: lerpColor(a.hillFront, b.hillFront),
    conifer: lerpColor(a.conifer, b.conifer),
    deciduous: lerpColor(a.deciduous, b.deciduous),
    trunk: lerpColor(a.trunk, b.trunk)
  };
}

function drawFlatCloud(x, y, scale, color) {
  cx.fillStyle = color;
  cx.beginPath();
  const w = 40 * scale;
  const h = 16 * scale;
  ...
  cx.beginPath();
  cx.arc(x - 4 * scale, y - 6 * scale, 10 * scale, 0, Math.PI * 2);
  cx.arc(x + 8 * scale, y - 4 * scale, 8 * scale, 0, Math.PI * 2);
  cx.fill();
}


function drawFlatFlower(x, y, isYellow, colors) {
  cx.save();
  cx.strokeStyle = colors.conifer;
  cx.lineWidth = 1;
  cx.beginPath();
  cx.moveTo(x, y);
  cx.lineTo(x, y - 10);
  ...
  const cy_blossom = y - 11;
  if (isYellow) {
    cx.fillStyle = '#fdb813';
    for (let i = 0; i < 5; i++) {
      const a = (i / 5) * Math.PI * 2;
      cx.beginPath();
      cx.arc(x + Math.cos(a) * 3, cy_blossom + Math.sin(a) * 3, 2, 0, Math.PI * 2);
      cx.fill();
    }
    cx.fillStyle = '#ffffff';
    cx.beginPath(); cx.arc(x, cy_blossom, 1.5, 0, Math.PI * 2); cx.fill();
  } else {
    cx.fillStyle = '#ffffff';
    for (let i = 0; i < 5; i++) {
      const a = (i / 5) * Math.PI * 2;
...
function drawFlatGrassTuft(x, y, colors) {
  cx.save();
  cx.strokeStyle = colors.conifer;
  cx.lineWidth = 1;
  cx.beginPath();
  cx.moveTo(x, y); cx.lineTo(x - 2, y - 5);
  cx.moveTo(x, y); cx.lineTo(x, y - 6);
  cx.moveTo(x, y); cx.lineTo(x + 2, y - 4);
  cx.stroke();
  cx.restore();
}
...
function drawTitleBg(W, H) {
  ...
  // Sky
  cx.fillStyle = colors.sky;
  cx.fillRect(0, 0, VW, VH);
  
  // Sun
  if (colors.sun) {
    cx.fillStyle = colors.sun;
    cx.beginPath();
    cx.arc(VW * 0.18, VH * 0.18, 28, 0, Math.PI * 2);
    cx.fill();
  }
  
  // Moon
  if (colors.moon) {
    cx.fillStyle = colors.moon;
    cx.beginPath();
    cx.arc(VW * 0.82, VH * 0.18, 14, 0, Math.PI * 2);
    cx.fill();
    cx.fillStyle = colors.sky;
    cx.beginPath();
    cx.arc(VW * 0.82 - 6, VH * 0.18 - 3, 12, 0, Math.PI * 2);
    cx.fill();
  }
  
  // Clouds
  const cloudCol = hexLerp('#ffffff', '#2a3a5c', pc.night);
  drawFlatCloud(VW * 0.08, VH * 0.32, 0.9, cloudCol);
  drawFlatCloud(VW * 0.78, VH * 0.14, 1.3, cloudCol);
  drawFlatCloud(VW * 0.88, VH * 0.38, 0.85, cloudCol);
  
  // Background Mountain Range
  cx.fillStyle = colors.mountainBack;
  cx.beginPath();
  cx.moveTo(-10, VH);
  cx.lineTo(-10, VH * 0.6);
  cx.lineTo(VW * 0.22, VH * 0.38);
  cx.lineTo(VW * 0.44, VH * 0.65);
  cx.lineTo(VW * 0.64, VH * 0.45);
  cx.lineTo(VW * 0.78, VH * 0.65);
  cx.lineTo(VW * 0.90, VH * 0.52);
  cx.lineTo(VW + 10, VH * 0.65);
  cx.lineTo(VW + 10, VH);
  cx.closePath(); cx.fill();
  
  // Foreground Mountain Range
  cx.fillStyle = colors.mountainFront;
  cx.beginPath();
  cx.moveTo(-10, VH);
  cx.lineTo(-10, VH * 0.7);
  cx.lineTo(VW * 0.12, VH * 0.54);
  cx.lineTo(VW * 0.38, VH * 0.75);
  cx.lineTo(VW * 0.54, VH * 0.7);
  cx.lineTo(VW * 0.74, VH * 0.42);
  cx.lineTo(VW + 10, VH * 0.78);
  cx.lineTo(VW + 10, VH);
  cx.closePath(); cx.fill();
  
  // Snowy Cap on Peak 5
  cx.fillStyle = hexLerp('#ffffff', '#1b2a47', pc.night);
  cx.beginPath();
  cx.moveTo(VW * 0.74, VH * 0.42);
  cx.lineTo(VW * 0.74 - 15, VH * 0.49);
  cx.lineTo(VW * 0.74 - 5, VH * 0.48);
  cx.lineTo(VW * 0.74, VH * 0.50);
  cx.lineTo(VW * 0.74 + 5, VH * 0.48);
  cx.lineTo(VW * 0.74 + 18, VH * 0.49);
  cx.closePath(); cx.fill();
  
  // Meadow Hill 1 (Back)
  cx.fillStyle = colors.hillBack;
  cx.beginPath();
  cx.moveTo(-10, VH * 0.78);
  cx.quadraticCurveTo(VW * 0.4, VH * 0.72, VW + 10, VH * 0.76);
  cx.lineTo(VW + 10, VH + 10);
  cx.lineTo(-10, VH + 10);
  cx.closePath(); cx.fill();
  
  // Meadow Hill 2 (Middle)
  cx.fillStyle = colors.hillMid;
  cx.beginPath();
  cx.moveTo(-10, VH * 0.83);
  cx.quadraticCurveTo(VW * 0.6, VH * 0.77, VW + 10, VH * 0.81);
  cx.lineTo(VW + 10, VH + 10);
  cx.lineTo(-10, VH + 10);
  cx.closePath(); cx.fill();
  
  // Meadow Hill 3 (Foreground)
  cx.fillStyle = colors.hillFront;
  cx.beginPath();
  cx.moveTo(-10, VH * 0.88);
  cx.quadraticCurveTo(VW * 0.3, VH * 0.84, VW + 10, VH * 0.87);
  cx.lineTo(VW + 10, VH + 10);
  cx.lineTo(-10, VH + 10);
  cx.closePath(); cx.fill();
  
  // Trees
  drawTitleTree(VW * 0.08, VH * 0.82, 0, 0.85, pc.night); // Larch left
  drawTitleTree(VW * 0.16, VH * 0.81, 1, 0.65, pc.night); // Spruce left
  drawTitleTree(VW * 0.91, VH * 0.82, 1, 0.82, pc.night); // Spruce right
  drawTitleTree(VW * 0.84, VH * 0.81, 0, 0.62, pc.night); // Larch right
  
  // Flowers & Grass
  drawFlatFlower(VW * 0.19, VH * 0.87, true, colors);
  drawFlatFlower(VW * 0.68, VH * 0.865, false, colors);
  drawFlatGrassTuft(VW * 0.14, VH * 0.87, colors);
  drawFlatGrassTuft(VW * 0.48, VH * 0.86, colors);
  drawFlatGrassTuft(VW * 0.79, VH * 0.87, colors);
  
  // Boot & Book
  drawIcon('boots', VW * 0.26, VH * 0.82, 24);
  drawIcon('book', VW * 0.36, VH * 0.82, 18);
  ...
  if (pc.night > 0.05) {
    for (let i = 0; i < 6; i++) {
      ...
      const fy = VH * 0.8 - 10 - ((i * 93 + frame * 0.08) % 40) + Math.sin(frame * 0.04 + i) * 3.5;
...
function drawTitle() {
  ...
  // Title Typography
  const titleY = H * 0.22;
  const titleColor = hexLerp('#26523c', '#fbf8f0', curPC.night);
  const subtitleColor = hexLerp('#26523c', '#e2dcca', curPC.night);
  
  setTracking(Math.min(6, W * 0.01));
  cx.fillStyle = titleColor;
  cx.font = `800 ${Math.min(62, W * 0.09)}px "Outfit", sans-serif`;
  cx.fillText(TX.title, W / 2, titleY);

  // Subtitle
  setTracking(Math.min(2, W * 0.003));
  cx.font = `600 ${Math.min(13, W * 0.02)}px "Outfit", sans-serif`;
  cx.fillStyle = subtitleColor;
  const subText = TX.subtitle.toUpperCase().replace(/·/g, '•');
  cx.fillText(subText, W / 2, titleY + 36);
  setTracking(0);

  // Fullscreen Button
  BTNS = [];
  if (fsSupported) {
    addBtn('fs', W - 40, 40, 18, isFullscreen() ? 'i:shrink' : 'i:expand');
    const b = BTNS[BTNS.length - 1];
    const on = hoveredId === 'fs';
    cx.fillStyle = on ? '#e2dcca' : '#fbf8f0';
    cx.strokeStyle = '#2b533e';
    cx.lineWidth = 1.2;
    cx.beginPath(); cx.arc(b.lx, b.ly, b.lr, 0, Math.PI * 2); cx.fill(); cx.stroke();
    cx.strokeStyle = '#2b533e'; cx.lineWidth = 2; cx.lineCap = 'round'; cx.lineJoin = 'round';
    cx.save();
    cx.translate(b.lx, b.ly);
    const sz = 6;
    if (isFullscreen()) {
      cx.beginPath();
      cx.moveTo(sz, -sz); cx.lineTo(2, -2); cx.lineTo(sz, -2); cx.moveTo(2, -2); cx.lineTo(2, -sz);
      cx.moveTo(-sz, -sz); cx.lineTo(-2, -2); cx.lineTo(-sz, -2); cx.moveTo(-2, -2); cx.lineTo(-2, -sz);
      cx.moveTo(sz, sz); cx.lineTo(2, 2); cx.lineTo(sz, 2); cx.moveTo(2, 2); cx.lineTo(2, sz);
      cx.moveTo(-sz, sz); cx.lineTo(-2, 2); cx.lineTo(-sz, 2); cx.moveTo(-2, 2); cx.lineTo(-2, sz);
      cx.stroke();
      ...
      cx.beginPath();
      cx.moveTo(2, -2); cx.lineTo(sz, -sz); cx.lineTo(sz - 3, -sz); cx.moveTo(sz, -sz); cx.lineTo(sz, -sz + 3);
      cx.moveTo(-2, -2); cx.lineTo(-sz, -sz); cx.lineTo(-sz + 3, -sz); cx.moveTo(-sz, -sz); cx.lineTo(-sz, -sz + 3);
      cx.moveTo(2, 2); cx.lineTo(sz, sz); cx.lineTo(sz - 3, sz); cx.moveTo(sz, sz); cx.lineTo(sz, sz - 3);
      cx.moveTo(-2, 2); cx.lineTo(-sz, sz); cx.lineTo(-sz + 3, sz); cx.moveTo(-sz, sz); cx.lineTo(-sz, sz - 3);
      cx.stroke();
  ...
  const bw2 = Math.min(280, W * 0.72);
  let rowY = H * 0.52;
  
  const formatBtnLabel = (label) => {
    return label.toUpperCase().replace(/·/g, '•');
  };
  
  const drawPrimaryBtn = (id, label, y) => {
    ...
    if (isHovered) {
      ...
      cx.scale(1.03, 1.03);
      cx.translate(-W / 2, -y);
      cx.shadowColor = 'rgba(38, 82, 60, 0.35)';
      cx.shadowBlur = 8;
      cx.shadowOffsetY = 2;
    }
    cx.fillStyle = '#2b533e';
    roundRect(W / 2 - bw2 / 2, y - 22, bw2, 44, 14);
    cx.fill();
    
    cx.fillStyle = '#ffffff';
    cx.font = '800 13px "Outfit", sans-serif';
    cx.textAlign = 'center';
    cx.textBaseline = 'middle';
    cx.fillText(formatBtnLabel(label), W / 2, y + 0.5);
    cx.restore();
    
    BTNS.push({ id, x: (W / 2) * DPR, y: y * DPR, w: bw2 * DPR, h: 44 * DPR, lr: 0 });
  };
  
  const drawSecondaryBtn = (id, label, y) => {
    const isHovered = hoveredId === id;
    cx.save();
    if (isHovered) {
      cx.translate(W / 2, y);
      cx.scale(1.03, 1.03);
      cx.translate(-W / 2, -y);
      cx.shadowColor = 'rgba(38, 82, 60, 0.2)';
      cx.shadowBlur = 6;
      cx.shadowOffsetY = 1;
    }
    cx.fillStyle = isHovered ? '#f0ebd8' : 'rgba(251, 248, 240, 0.85)';
    cx.strokeStyle = '#2b533e';
    cx.lineWidth = 1.5;
    roundRect(W / 2 - bw2 / 2, y - 19, bw2, 38, 12);
    cx.fill(); cx.stroke();
    
    cx.fillStyle = '#2b533e';
    cx.font = '800 12px "Outfit", sans-serif';
    cx.textAlign = 'center';
    cx.textBaseline = 'middle';
    cx.fillText(formatBtnLabel(label), W / 2, y + 0.5);
    cx.restore();
  ...
  const drawLanguageBtn = (id, label, y) => {
    const isHovered = hoveredId === id;
    cx.save();
    if (isHovered) {
      cx.translate(W / 2, y);
      cx.scale(1.03, 1.03);
      cx.translate(-W / 2, -y);
      cx.shadowColor = 'rgba(38, 82, 60, 0.15)';
      cx.shadowBlur = 6;
      cx.shadowOffsetY = 1;
    }
    cx.fillStyle = isHovered ? '#f0ebd8' : '#fbf8f0';
    cx.strokeStyle = '#2b533e';
    cx.lineWidth = 1.2;
    roundRect(W / 2 - bw2 / 2, y - 18, bw2, 36, 12);
    cx.fill(); cx.stroke();
    ...
    cx.lineJoin = 'round';
    cx.beginPath();
    cx.moveTo(W / 2 + bw2 / 2 - 22, y - 2);
    cx.lineTo(W / 2 + bw2 / 2 - 18, y + 2);
    cx.lineTo(W / 2 + bw2 / 2 - 14, y - 2);
    cx.stroke();
  ...
  if (hasSave()) {
    rowY = H * 0.48;
    drawPrimaryBtn('cont', TX.title_continue, rowY); rowY += 52;
    drawSecondaryBtn('new', TX.title_new, rowY); rowY += 46;
  } else {
    rowY = H * 0.52;
    drawPrimaryBtn('start', TX.title_start, rowY); rowY += 56;
  }
  
  const langLabel = LANG === 'de' ? 'SPRACHE: DEUTSCH' : 'LANGUAGE: ENGLISH';
  drawLanguageBtn('lang', langLabel, rowY);
  
  // Bottom Controls Legend
  const hy = H - 48;
  const legend = LANG === 'de' ? [
    { keys: ['←', '→', '↑'], label: 'LAUFEN' },
    { keys: ['SPACE'], label: 'SPRINGEN' },
    { keys: ['E'], label: 'BENUTZEN' },
    { keys: ['M'], label: 'KARTE' },
  ...
  cx.save();
  let totalW = 0;
  cx.font = 'bold 9px "Outfit", sans-serif';
  for (const item of legend) {
    let w = 0;
    for (const k of item.keys) {
      w += Math.max(16, cx.measureText(k).width + 8) + 3;
    }
    w -= 3 + 6;
    cx.font = '600 10px "Outfit", sans-serif';
    w += cx.measureText(item.label).width;
    totalW += w + 24;
    cx.font = 'bold 9px "Outfit", sans-serif';
  }
  totalW -= 24;
  ...
  for (let i = 0; i < legend.length; i++) {
    ...
    for (const k of item.keys) {
      cx.font = 'bold 9px "Outfit", sans-serif';
      const kw = Math.max(16, cx.measureText(k).width + 8);
      const kh = 16;
      const ky = hy - kh / 2;
      ...
      cx.lineWidth = 1;
      cx.beginPath();
      cx.roundRect(curX, ky, kw, kh, 3);
      cx.fill(); cx.stroke();
      
      cx.fillStyle = 'rgba(255, 255, 255, 0.9)';
      cx.textAlign = 'center';
      cx.fillText(k, curX + kw / 2, hy + 0.5);
      curX += kw + 3;
    }
    curX -= 3;
    curX += 6;
    ...
    cx.textAlign = 'left';
    cx.fillText(item.label, curX, hy + 0.5);
    curX += cx.measureText(item.label).width;
  ...
  // Footer description
  cx.font = '600 9px "Outfit", sans-serif';
  cx.fillStyle = 'rgba(255, 255, 255, 0.5)';
  cx.textAlign = 'center';
  cx.fillText('A HIKE, NOT A FIGHT • EIN SPAZIERGANG, KEIN KAMPF.', W / 2, hy + 20);
